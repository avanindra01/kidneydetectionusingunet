...
test_dir = r"C:\Users\Conor\Documents\kidneyproject\test"
#mask_test_dir = r"C:\Users\Conor\Documents\kidneyproject\train\mask\test"
inner_file_list = os.listdir(os.path.join(test_dir,'small'))

testGene = testGenerator(os.path.join(test_dir,'small'),num_image=len(inner_file_list))
for i in range(len(inner_file_list)):
    cap = next(testGene)
    ...
    #plt.figure()
    #out_mask[:,:,0] = mask
    out_mask= (results[0,:,:,0]>0.1)*254
    cv2.imwrite(os.path.join(test_dir,'masks',inner_file_list[i]),out_mask)
    if i%100==0:
        print(i)
